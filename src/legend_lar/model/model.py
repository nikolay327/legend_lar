...
from legend_lar.model.tokenizer import DiscreteEmbedder, ContinuousEmbedder, JointHPGeEmbedder, HPGewithPSD
...
class AttnContrastiveRatioEstimator(ConditionalRatioEstimator):
    def __init__(
        self,
        config: BootstrappedKFoldConfig,
        device
    ):
        super(AttnContrastiveRatioEstimator, self).__init__(config, device)
        self.joint_hpge_emb = HPGewithPSD(
            emb_dim=self.config.hidden_size,
            hpge_codebook_size=self.config.num_hpges,
            global_partitioning_size=self.config.global_partitioning_size,
            hidden_dim=self.config.intermediate_size,
            num_of_features=self.config.num_hpge_features,
            num_attn_heads=self.config.num_attention_heads,
            num_layers=self.config.attn_num_hpge_emb_layers
        )
    def forward(
        self,
        g: Tensor, # (B / 2,)
        ...
        if e_hpge_ is None:
            e_hpge = self.joint_hpge_emb(g, p, E) # (B / 2, D)
