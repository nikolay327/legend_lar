...
class ContrastiveTrainer(TrainerBase):
    ...
    def reset_model_and_optimizer(self):
        self.model = ContrastiveRatioEstimator(
            config=self.config,
            device=self.device
    ...
    def calculate_loss(self, pos_logits: Tensor, neg_logits: Tensor):
        B = self.config.local_batch_size // 2
        K = len(pos_logits)
        logK = math.log(K)
        loggamma = math.log(self.config.gamma)
        # dependent
        logits_dep = torch.cat([pos_logits, neg_logits[:, :K-1]], dim=1) # (B, K)
        # indenpendent class at 0
        logits_dep = torch.cat(
            [
                torch.full((B, 1), logK, device=pos_logits.device),
                logits_dep + loggamma
            ],
            dim=1
        ) # (B, K+1)
        targets_dep = torch.full((B,), 1, dtype=torch.long, device=pos_logits.device) # positive class is 1
        loss_dep = F.cross_entropy(logits_dep, targets_dep)

        # independent
        logits_ind = torch.cat(
            [
                torch.full((B,1), logK, device=pos_logits.device),
                neg_logits + loggamma
            ],
        ...
        targets_ind = torch.zeros((B,), dtype=torch.long, device=pos_logits.device) # independent class
        loss_ind = F.cross_entropy(logits_ind, targets_ind)

        loss = (1.0/(1.0+self.config.gamma))*loss_ind + (self.config.gamma/(1.0+self.config.gamma))*loss_dep
        return loss
    ...
    def forward_batch(
        ...
        e_hpge = e_hpge.to(torch.float32)
        ...
        loss = self.calculate_loss(pos_logits, neg_logits)
        return loss
...
def train_contrastive(experiment: str, model_name: str, version: str, working_dir: str, data_dir: str, training_config: str):
