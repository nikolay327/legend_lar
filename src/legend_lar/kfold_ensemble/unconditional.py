...
class UnconditionalTrainer(TrainerBase):
    def __init__(
        self,
        config: BootstrappedKFoldConfig,
        device: str | int
    ):
        super(UnconditionalTrainer, self).__init__(config, device)

    def _set_model_initializer(self):
        self.rng_seed_for_unconditional_model_init = self.BASE_RNG.getrandbits(64)
        self.rng_seed_for_conditional_model_init = self.BASE_RNG.getrandbits(64) # This is here for consistency

        self.model_initiator = InitRNG(
            seed=self.rng_seed_for_unconditional_model_init, device=self.device
        )

    def reset_model_and_optimizer(self):
        self.model = UnconditionalRatioEstimator(
            config=self.config,
            device=self.device
...
def train_unconditional(experiment: str, model_name: str, version: str, working_dir: str, data_dir: str, training_config: str):
    ...
    config.hpge_id_and_energy = None
...
if __name__ == "__main__":
    ...
    train_unconditional(args.experiment, args.model_name, args.version, args.working_dir, args.data_dir, args.training_config)
